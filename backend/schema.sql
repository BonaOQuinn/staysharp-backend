...
--Appointments 
--rule:appointments.location_id MUST match the barbers locatation_id
...
CREATE TABLE IF NOT EXISTS users (
    ...
    --permission
    role TEXT NULL CHECK (role IN ('barber', 'owner')), 
    
    --isolation: barber belongs to a location; owner can be null
    location_id INT REFERENCES locatations(id) ON DELETE SET NULL, 
...
CREATE INDEX IF NOT EXISTS appointments_barber_start_end_idx ON appointments (barber_id, start_ts, end_ts)
